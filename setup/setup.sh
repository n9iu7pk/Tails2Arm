...
#          Bugfix missing quotes (end of script, summary output)
...
# 20160821 reworked, i.e. reuse of img-file, detection of deboostrap stages etc.
#          changed img size from 5 to 15 GB
...
. ./setup/tailsDevFunc.sh
log "'. ./setup/tailsDevFunc.sh' executed"
if [ $? -ne 0 ]; then
	echo "[$(date +"%Y-%m-%d %H:%M:%S")] ABORT(-1): '. ./setup/tailsDevFunc.sh' not executed." 1>&2
	exit 1
fi
...
export LUKSNAME="$(echo "$1" | cut -d':' -f1)"
export PASSWORD="$(echo "$1:" | cut -d':' -f2)"
...
if (grep "^aliases=${SCHROOT}$" /etc/schroot/chroot.d/* 1>/dev/null 2>&1); then
	abortOnFailure "9" "Alias '${SCHROOT}' already exists, see '/etc/schroot/chroot.d/*'. To solve, remove file /etc/schroot/chroot.d/${SCHROOT}.conf'"
fi
if (grep "^aliases=${SCHROOT}$" /etc/schroot/schroot.conf 1>/dev/null 2>&1); then
	abortOnFailure "9" "Alias '${SCHROOT}' already exists, see '/etc/schroot/schroot.conf'. To solve, remove alias from '/etc/schroot/schroot.conf'"
fi
...
echo " - sharutilsn          uuencode"
if [ "${QEMU}" == "YES" ]; then
	echo " - qemu-user-static (installs qemu-debootstrap)"
fi
echo "schroot name:          ${SCHROOT}"
echo "Architecture (chroot): ${ARCH}"
echo "Mount point:           '$(pwd)/${LUKSNAME}'"
if [ "${PASSWORD}" != "" ]; then
	echo "Passphrase             '${PASSWORD}'"
else
	echo "Passphrase             /dev/urandom"
fi
echo "schroot config file:   '/etc/schroot/chroot.d/${SCHROOT}.conf' will be created."
...
abortOnFailure "$?" "apt-get --force-yes -y update"
# BUT: *NEVER*EVER* do apt-get upgrade. 
# This must be unter control of the system owner.
apt-get --force-yes -y install btrfs-tools schroot sbuild debootstrap fakeroot eatmydata dmsetup cryptsetup cryptsetup-bin sharutils
abortOnFailure "$?" "apt-get --force-yes -y install btrfs-tools schroot sbuild debootstrap fakeroot eatmydata dmsetup cryptsetup cryptsetup-bin sharutils"
if [ "${QEMU}" == "YES" ]; then
	# If you choose a qemu emulated arm chroot, uncomment the next lines
	log "Install qemu-user-static (installs qemu-debootstrap)"
	apt-get --force-yes -y install qemu-user-static
	abortOnFailure "$?" "apt-get --force-yes -y install qemu-user-static"
fi 
...
export loopDev=$(losetup -a | grep "${img}" | tail -1 | cut -d':' -f1)
abortOnFailure "$?" "losetup -a | grep \"${img}\" | tail -1 | cut -d':' -f1"
...
if [ "${PASSWORD}" == "" ]; then
	...
	log "Encrypting image ${img} using predefined passphrase"
fi
# LUKS1 defaults
...
log "Check encryption of image ${img} with luksOpen ... "
cryptsetup -q luksOpen "${loopDev}" "${LUKSNAME}" << EOCRYPT
${PASSWORD}
EOCRYPT
if [ $? -ne 0 ]; then
	rm -rf "../${LUKSNAME}".deboostrap.*
	log "... must be encrypted."
...
EOCRYPT
	...
	log "... already encrypted and opened."
fi
...
# Create a mount point and check filesystem for btrfs
log "Create mount point ${mountPoint}"
mkdir -m 777 -p "${mountPoint}"
abortOnFailure "$?" "mkdir -m 777 -p \"${mountPoint}\""
if !(mount -t btrfs "${loopDevCrypt}" "${mountPoint}"); then
	rm -rf "../${LUKSNAME}".deboostrap.*
	...
	log "Encrypted image ${img} already formated with btrfs"
fi
cd "${mountPoint}"
...
if [ $(ls ../build/*.sh | wc -l) -gt 0 ]; then
	cp ../build/*.sh .
	abortOnFailure "$?" 'cp ../build/*.sh'
fi
if [ $(ls ../build/*.key | wc -l) -gt 0 ]; then
	cp ../build/*.key .
...
if [ ! -f "../${LUKSNAME}.deboostrap.first-stage" ]; then
	...
	echo "$(date +"%Y-%m-%d %H:%M:%S")" >"../${LUKSNAME}.deboostrap.first-stage"
fi

if [ ! -f "../${LUKSNAME}.deboostrap.second-stage" ]; then
	...
	echo "$(date +"%Y-%m-%d %H:%M:%S")" >"../${LUKSNAME}.deboostrap.second-stage"
fi
...
# - install vim
log "Chroot ${SCHROOT}: apt-get --force-yes -y install devscripts vim-tiny"
schroot -c "source:${SCHROOT}" -u root --directory=/root <<EOSCHROOT
apt-get --force-yes -y install devscripts vim-tiny
EOSCHROOT
abortOnFailure "$?" "Chroot ${SCHROOT}: apt-get --force-yes -y install devscripts vim-tiny"
# - apt mirrors
...
# Generate u/mount script
cat > "${LUKSNAME}.sh"<<EOS
#! /bin/bash

export password="${PASSWORD}"
export volume="${LUKSNAME}"
case "\$1" in
	"ON")
		if !(mount | grep "\$(pwd)/\${volume} " >/dev/null); then
			./setup/mount.sh "\${volume}" <<EOM
\${password}
EOM
		fi
		if (mount | grep "\$(pwd)/\${volume} " >/dev/null); then
			echo "Volume \${volume} mounted." 1>&2
		else
			echo " *** FAILED: Mount volume \${volume} *** " 1>&2
		fi
		;;
	"OFF")
		if (mount | grep "\$(pwd)/\${volume} " >/dev/null); then
			./setup/umount.sh "\${volume}"
		fi
		if !(mount | grep "\$(pwd)/\${volume} " >/dev/null); then
			echo "Volume \${volume} unmounted." 1>&2
		else
			echo " *** FAILED: Unmount volume \${volume} *** " 1>&2
		fi
		;;
	"STATE")
		if (mount | grep "\$(pwd)/\${volume} " >/dev/null); then
			...
			echo "Volume \${volume} NOT mounted." 1>&2
		fi
		;;
	"DELETE")
		if !(mount | grep "$(pwd)/\${volume} " >/dev/null); then
			...
			rm -rf "\${volume}.img"
			rm -rf "\${volume}.log"
			rm -rf "/etc/schroot/chroot.d/\${SCHROOT}.conf"
			echo "Volume \${volume} removed" 1>&2
		else
			echo " *** FAILED: Volume \${volume} still mounted *** " 1>&2
		fi
		;;
	*)
		echo "usage: \$0 CMD" 1>&2
		echo "CMD    ON     = mount \${volume}" 1>&2
		echo "       OFF    = un-mount \${volume}" 1>&2
		echo "       STATE  = mount state of \${volume}" 1>&2
		echo "       DELETE = remove volume \${volume} and files" 1>&2
		;;	
esac
EOS
chmod 755 "${LUKSNAME}.sh"
...
echo ""
echo "To get the mount state execute from '$(pwd)':"
echo "   ${LUKSNAME}.sh STATE"
echo ""
...
echo "   ${LUKSNAME}.sh OFF"
echo "   umount.sh \"${LUKSNAME}\""
echo ""
echo "To clean up:"
echo "   rm -rf ./setup/"
echo "   rm -rf ANY LOGFILES"
echo "   # Verify that all ./build/*.sh are copied to ./${LUKSNAME}"
echo "   rm -rf ./build/"
echo ""
echo "Enjoy :)"
